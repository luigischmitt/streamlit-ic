...
tab1, tab2 = st.tabs(['Sobre', 'Linguagens'])

with tab1:
    st.title("Sobre nós")

    st.image("images/Brasão_UFPB.png", use_column_width=False, width=100)

    st.markdown("""
    Este trabalho foi desenvolvido como parte da disciplina de **Introdução à Ciência da Computação** da **Universidade Federal da Paraíba (UFPB)**.
    
    O projeto tem como objetivo explorar o futuro das principais linguagens de programação, abordando suas tendências e as áreas em que devem continuar a se destacar nos próximos anos.
    ...
    Esperamos que você encontre informações úteis e inspiradoras sobre as linguagens de programação que movem o mundo da tecnologia! 👋🏻
    """)
